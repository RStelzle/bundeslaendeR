...
Note that polidoc.net provides a coalition agreement between the SPD and the Greens following the 2008 HE election (41001.006.2008.1.1). Since this potential coalition under leadership of SPD politician Andrea Ypsilanti never came to be due to several SPD MP's opposing the red-green minority cabinet being externally supported by Die Linke the coalition agreement can't be matched with a government in `ltw_election_results_and_gov` and is thus not included.
...
ltw_election_results %>% 
  ...
  mutate(no_cat = fct_case_when(
    number_of_manif == 0 ~ "No manifesto available",
    number_of_manif > 0 & number_of_extra_manif == 0 ~ "At least one manifesto available",
    number_of_extra_manif > 0 ~ "At least one party with two available manifestos"
  )) %>% 
