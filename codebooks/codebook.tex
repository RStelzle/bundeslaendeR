...
\ofoot*{14.05.2022} \ihead{\headmark}
...
\texttt{bundeslaendeR::ltw\_elections} returns data frame (tibble if the
tibble package is loaded) containing one row per contesting party per
election. For a schematic version of
\texttt{bundeslaendeR::ltw\_elections}'s structure see
...
\texttt{ltw\_governments} returns a data frame (tibble if the tibble
package is loaded) containing information on governments in the German
states. Each row contains information on one state government.

\hypertarget{ltw_elections-variable-information-1}{%
\subsection{\texorpdfstring{\texttt{ltw\_elections} Variable
Information}{ltw\_elections Variable Information}}\label{ltw_elections-variable-information-1}}
...
\begin{flushleft}\vspace{-0.8cm}Type of Government:\setlist{nolistsep}\begin{itemize}[noitemsep]\item Single Party Majority \item Oversized Coalition \item Minimal Winning Coalition \item Single Party Minority \item Multi Party Majority \item Caretaker. \end{itemize}
\end{flushleft}
\end{longtable}
...
\texttt{ltw\_combined} returns a returns data frame (tibble if the
tibble package is loaded) containing both election results as well as
linked information on governments in the German states. Each row
contains information on one party during the time in office of one
cabinet. For a schematic version of
\texttt{bundeslaendeR::ltw\_combined}'s structure see
...
Is the governments minister president from this party? Note: There is a single cabinet where the minister president is not part of any party: Heinrich Welsch's caretaker government in the Saarland (at the time not yet a member of the FRG) in 1955.
\end{longtable}
...
\texttt{ltw\_elections\_meta} returns a returns data frame (tibble if
the tibble package is loaded) containing both meta information on
election results. Each row contains information on one election.
...
In the following section \(v_i\) refers to party \(i\)'s vote share,
\(s_i\) to party \(i\)'s seat share and \(n\) refers to the number of
parties contesting a given election.
...
Effective number of parties in the electorate $N_{2 electorate}$ \parencite{laaksoEffectiveNumberParties1979}:
...
Effective number of parties in parliament $N_{2 parliament}$ \parencite{laaksoEffectiveNumberParties1979}:
