...
if (isFALSE(here("codebooks", "fntdl", "libertinus.zip"))) {
...
`bundeslaendeR::ltw_elections` returns data frame (tibble if the tibble
package is loaded) containing one row per contesting party per election.
For a schematic version of `bundeslaendeR::ltw_elections`'s structure see table$~$\ref{strltwelecres}.
...
item_start(item_name_code = "ppeg_id", 
           item_name = "PPEG ID",
           item_desc = "If available, party id of the party in the PPEG database \\parencite{ppegDatabasePoliticalParties2022}. These party IDs are chiefly based on party IDs from \\textcite{mackieInternationalAlmanacElectoral1991}.")
```
...
`ltw_governments` returns a data frame (tibble if the tibble
package is loaded) containing information
...
short_item(item_name_code = "gov_tog",
           ...
           item_desc = "\\vspace{-0.8cm}Type of Government:\\setlist{nolistsep}\\begin{itemize}[noitemsep]\\item Single Party Majority \\item Oversized Coalition \\item Minimal Winning Coalition \\item Single Party Minority \\item Multi Party Majority \\item Caretaker. \\end{itemize}",
           desc_left = TRUE)
```
...
`ltw_combined` returns a returns data frame (tibble if the tibble
...
short_item(item_name_code = "is_mp_party",
           ...
           item_desc = "Is the governments minister president from this party? Note: There is a single cabinet where the minister president is not part of any party: Heinrich Welsch's caretaker government in the Saarland (at the time not yet a member of the FRG) in 1955.",
           desc_left = FALSE)
```
...
`ltw_elections_meta` returns a returns data frame (tibble if the tibble
package is loaded) containing both meta information on election results.
Each row contains information on one election.
...
In the following section $v_i$ refers to party $i$'s vote share, $s_i$ to party $i$'s seat share and $n$ refers to the number of parties contesting a given election.
...
item_start(item_name_code = "fragmentation_enep",
           ...
           item_desc = "Effective number of parties in the electorate $N_{2 electorate}$ \\parencite{laaksoEffectiveNumberParties1979}:
           \\begin{equation}N_{2 electorate} = \\frac{1}{\\sum_{i = 1}^{n}v_{i}^{2}}. \\end{equation}",
           desc_left = FALSE)
```
...
item_start(item_name_code = "fragmentation_enpp",
           ...
           item_desc = "Effective number of parties in parliament $N_{2 parliament}$ \\parencite{laaksoEffectiveNumberParties1979}:
           \\begin{equation}N_{2 parliament} = \\frac{1}{\\sum_{i = 1}^{n}s_{i}^{2}}. \\end{equation}",
           desc_left = FALSE)
```
...
short_item(item_name_code = "disprop_rae",
           ...
           \\begin{equation}I_{Rae} = \\frac{1}{n}\\sum_{i = 1}^{n}|s_i - v_i|.\\end{equation}",
           desc_left = FALSE)
```
...
short_item(item_name_code = "disprop_\nloosmore_hanby",
           ...
           \\begin{equation}I_{LH} = \\frac{1}{2}\\sum_{i = 1}^{n}|s_i - v_i|.\\end{equation}",
           desc_left = FALSE)
```
...
short_item(item_name_code = "disprop_gallagher",
           ...
           item_desc = "Gallagher index of electoral disproportionality.",
           desc_left = FALSE)
```
...
ltw_elections_meta %>% 
  ...
  ggplot(aes(x = valstd, group = measure)) +
    geom_density(alpha = 0.6, col = "#810a26") +
...
# `link_manifestos` and `link_coalitionagreements`
...
cat("\\texttt{link\\_manifestos} and \\texttt{link\\_coalitionagreements} provide easy links of \\texttt{bundeslaendeR} data with party manifestos and coalition agreements made available from polidoc.net - The Political Documents Archive \\parencites{benoitChallengesEstimatingPolicy2009}{grossDoesEURegional2018}{pappiPolitikpositionenDeutschenLandtagsparteien2014}{pappiPartyElectionProgrammes2009}[for the codebook see][]{brauningerPolidocNetCodebook2018}.
While file names from polidoc.net follow a naming pattern
...
## Linking-Variables Information


The variables `state`, `election_date`, and `partyname_short` can be used in order to link manifestos to the `bundeslaendeR` data.
...
The variable `gov_id` can be used in order to link manifestos to the `bundeslaendeR` data.
...
```{r, results="asis"}
item_start(item_name_code = "polidoc_filename and polidoc_filename_2 in link_manifestos",
...
```{r, results="asis"}
item_start(item_name_code = "polidoc_filename in link_\ncoalitionagreements",
