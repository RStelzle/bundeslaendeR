...
biblatexoptions: [backend=biber,backref, style=ext-authoryear-comp,maxbibnames=99,maxcitenames=3,isbn=false,date=year,sorting=nyt]
...
Using the provided datasets `link_manifestos`, `link_coalitionagreements` and `link_positions_pwib` `bundeslaendeR` data can be linked to other datasets.
...
The variables `state`, `election_date`, and `partyname_short` can be used in order to link PWIB policy positions to the `bundeslaendeR` data using `link_coalitionagreements`. Note that the state abbreviations provided in the PWIB data file (`pwib2020_final.dta`) are not capitalized while the state abbreviation in `link_positions_pwib` are.


```{r, results="asis"}
item_start(item_name_code = "party_pwib in link_positions_pwib",
...
ltw_elections %>%
  ...
  left_join(tibble(state = sort(bundeslaendeR:::int_grid$code)) %>% mutate(y = row_number() %>% as.double())) %>%
  ...
  rename(
    "Parties with an available policy position" = covered,
    "Parties with no available policy position" = not_covered
  )
...
posplotdata %>%
  ggplot(aes(x = x, y = y)) +
    ...
    scale_y_continuous(breaks = 1:16, labels = sort(bundeslaendeR:::int_grid$code)) +
  ...
  scatterpie::geom_scatterpie(
    data = posplotdata,
    cols = rev(c("Parties with an available policy position", "Parties with no available policy position")), pie_scale = 0.8,
    color = NA
    ) +
  coord_equal() +
  labs(
