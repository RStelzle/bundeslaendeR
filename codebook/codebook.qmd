...
format:
  pdf:
    ...
    biblatexoptions: [style=ext-authoryear-comp,backend=biber,maxbibnames=99,maxcitenames=3,isbn=false,date=year,sorting=nyt]
...
knitr::opts_chunk$set(echo = FALSE, fig.path = "cbfiles/")
...
if (isFALSE(file.exists(here("codebook", "fntdl", "libertinus.zip")))) {
  dir.create(here("codebook", "fntdl"))
curl::curl_download("http://mirrors.ctan.org/fonts/libertinus-fonts.zip",
                    here("codebook", "fntdl", "libertinus.zip"))
  unzip(zipfile =  here("codebook", "fntdl", "libertinus.zip"), exdir = here("codebook", "fntdl"))
}


dir.create(here("codebook", "cbfiles"))
...
font_add("LibertinusSansSerif",
         regular = here("codebook", "fntdl", "libertinus-fonts", "otf", "LibertinusSans-Regular.otf"),
         bold = here("codebook", "fntdl", "libertinus-fonts", "otf", "LibertinusSans-Bold.otf"),
         italic = here("codebook", "fntdl", "libertinus-fonts", "otf", "LibertinusSans-Italic.otf"))
...
item_start(item_name_code = "decker_neu",
           ...
           item_desc = "Denotes, wether the Handbuch der deutschen Parteien (3. ed.) by \\textcite{deckerHandbuchDeutschenParteien2018b} has a chapter on the party.",
           desc_left = FALSE)
```
...
short_item(item_name_code = "disprop_gallagher",
          ...
          \\begin{equation}Lsq = \\sqrt{\\frac{1}{2}\\sum_{i = 1}^{n}\\left(s_i - v_i\\right)^2}.\\end{equation}",
           desc_left = FALSE)
```
...
```{r, results="asis"}
short_item(item_name_code = "disprop_\ngallagher_normalized",
           item_name = "Normalized Gallagher index of electoral disproportionality",
           item_desc = "Gallagher index of electoral disproportionality normalized by effective number of vote winning parties \\parencite{Bailey_2026}:
          \\begin{equation}I_{GNorm} = \\sqrt{\\frac{N_{2 \\text{ electorate}}}{N_{2 \\text{ electorate}} + 1}\\sum_{i = 1}^{n}\\left(s_i - v_i\\right)^2}.\\end{equation}",
           desc_left = FALSE)
```


```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{fig:distdisp}Distribution of Disproportionality Measures", fig.width=8*1.1, fig.height=7*1.1, label=distdisp}
...
Using the provided datasets `link_manifestos`, `link_coalitionagreements`, `link_positions_pwib`, `link_integrated_state_election_surveys`, `link_statepol_ppg`, `link_statepol_cabinets` and  `link_legcap_appeldorn_fortunato` `bundeslaendeR` data can be linked to other datasets.
...
\blandscape
```{r printmanifestoplot, fig.cap="\\label{figure:manifestoavailability}Availability of manifestos from polidoc.net and abgeordnetenwatch.de", fig.height=8*0.9, fig.width=12*0.9}
manifestoplots
...
`link_legcap_appeldorn_fortunato` provides a link between `bundeslaendeR` data and data on Legislative Capacity in Germany's Parliaments (10.7910/DVN/BA8G7H) provided by \textcite{fortunatoReplicationDataLegislative2021} \parencite[for more details see][]{appeldornLegislativeCapacityGermany2022},
...
Dataset providing a link between ltw_elections (or ltw_combined) and party names in vote choice in the integrated dataset of state election surveys availabel on GESIS (ZAZA4182) \parencite{scheuchGermanFederalState2015}. Currently linked to StatePol version 
...
Dataset providing a link between `ltw_elections` (or `ltw_combined`) and data on parliamentary party groups in the StatePol database \parencite{kochStatePolDatenbankMitgliedern2024a}. 

Use the `ppg` variable in the StatePol PPG dataset (not the `ppg_id` variable) to link StatePol and `bundeslaendeR` data. Party groups that are founded later in the legislative cycle cannot be linked to electoral outcome data. See `link_notes` for details on such cases. 

The Bündnis 90/Die Grünen (AL) / UFV party group links to the election results of both Bü90GrüUFV and Grüne in the 1990 Berlin state elections as MdLs running on both lists formed a joint party group immediately following the election.


```{r, results="asis"}
short_item(item_name_code = "landtag_state_abb",
           item_name = "State name abbreviation in StatePol",
           item_desc = "State name abbreviation in StatePol dataset.",
           desc_left = FALSE)
```
...
```{r, results="asis"}
short_item(item_name_code = "electoralperiod",
           item_name = "State electoral period in StatePol",
           item_desc = "State electoral period in StatePol dataset. Note that this count does not neccesarily match the number \\texttt{state\\_election\\_term}. See the describtion of the \\texttt{state\\_election\\_term} variable above.",
           desc_left = FALSE)
```


```{r, results="asis"}
short_item(item_name_code = "ppg",
           item_name = "Name of Parliamentary Party Group on StatePol",
           item_desc = "Name of Parliamentary Party Group on StatePol. Note that the \\texttt{ppg} column is different from the \\texttt{ppg\\_id} column in the original StatePol dataset.",
           desc_left = FALSE)
```
...
Dataset providing links between government data in `ltw_governments` and `ltw_combined` and cabinet data from StatePol database \parencite{kochStatePolDatenbankMitgliedern2024a}.

In some cases one cabinet in the StatePol database is assigned two `gov_ids` in `bundeslaendeR` data as StatePol covers parties leaving a coalition using time variant cabinet membership of party ministers whereas `bundeslaendeR` creates a new government entry. In these cases there are two values in the `gov_id` column, separated by a semi-colon.
...
```{r, results="asis"}
short_item(item_name_code = "cabinet",
           item_name = "Cabinet name in StatePol",
           item_desc = "Cabinet name in StatePol dataset.",
           desc_left = FALSE)
```
...
parliamentary speeches from the StateParl database (paragraphs dataset)
\parencite{baltermann_2025}. Party groups that are founded later in the
legislative cycle cannot be linked to electoral outcome data. See `link_notes` for
details on such cases.
Note that StateParl treats the time before and after the re-election of the
Berlin Abgeordnetenhaus in 2023 (re-running the 2021 election) as the same
electoral period. In the `bundeslaendeR` data the `state_election_term` count gets
incremented. The `election_date` variable in `link_stateparl` thus contains both
election dates for this electoral period, separated by a semi-colon. Also use
the the `date` variable from StateParl's paragraphs dataset to link the data for
this electoral period.
Note that StateParl's `affiliation` variable denotes the political position
instead of the party affiliation for some speeches, in particular for members of
the state parliament presidency and -more crucially- for members of the state
government. `link_stateparl` thus cannot not help with linking these politicians'
speeches to party level election results, etc. Please refer to the StateParl codebook for more information.


```{r, results="asis"}
short_item(item_name_code = "state",
           item_name = "State name abbreviation in StateParl",
           item_desc = "State name abbreviation in StateParl dataset.",
           desc_left = FALSE)
```

```{r, results="asis"}
short_item(item_name_code = "period",
           item_name = "State electoral period in StateParl",
           item_desc = "State electoral period in StateParl dataset.",
           desc_left = FALSE)
```

```{r, results="asis"}
short_item(item_name_code = "affiliation",
           item_name = "Party affiliation in StateParl",
           item_desc = "Speakers' party affiliation in StateParl dataset.",
           desc_left = FALSE)
```

```{r, results="asis"}
short_item(item_name_code = "state_bundeslaender",
           item_name = "State name abbreviation in bundeslaendeR",
           item_desc = "State name abbreviation in bundeslaendeR.",
           desc_left = FALSE)
```
...
Dataset providing a link between `ltw_elections` (or `ltw_combined`) and state election
results at the municipal level from GERDA - German Election Database \parencite{Heddesheimer2025GERDA}.


```{r, results="asis"}
short_item(item_name_code = "state_gerda",
           item_name = "State number in GERDA data",
           item_desc = "State number in GERDA data.",
           desc_left = FALSE)
```
...
```{r, results="asis"}
short_item(item_name_code = "partyname_gerda",
           item_name = "Party name in GERDA data",
           item_desc = "Party name in GERDA data.",
           desc_left = FALSE)
```
...
short_item(item_name_code = "partyname_short",
           item_name = "Abbreviated Party Name",
           item_desc = "Party abbreviation in bundeslaendeR data.",
           desc_left = FALSE)
```
