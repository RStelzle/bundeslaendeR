...
\fi
\usepackage[style=ext-authoryear-comp,backend=biber,maxbibnames=99,maxcitenames=3,isbn=false,date=year,sorting=nyt]{biblatex}
...
Harmonized abbreviation of the party's name. 394 unique parties.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{partyname} &\textbf{Party Name}\newline 
Harmonized name of the party. 394 unique parties.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{partyname\_short\_
bundeswahlleiter} &\textbf{Party Name Abbreviation from Bundeswahlleiter}\newline 
Partyname abbreviation as documented by the Bundeswahlleiter. 486 different abbreviations.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{partyname\_
bundeswahlleiter} &\textbf{Party Name from Bundeswahlleiter}\newline 
Partyname as documented by the Bundeswahlleiter. 531 different names.
\end{longtable*}
...
If available, party id of the party in the PPEG database \parencite{krausePPEGPoliticalParties2024a}. These party IDs are chiefly based on party IDs from \textcite{mackieInternationalAlmanacElectoral1991}.
...
Denotes, wether the Handbuch der deutschen Parteien (3. ed.) by \textcite{deckerHandbuchDeutschenParteien2018b} has a chapter on the party.
...
\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{disprop\_
gallagher\_normalized} &\textbf{Normalized Gallagher index of electoral disproportionality}\newline 
Gallagher index of electoral disproportionality normalized by effective number of vote winning parties \parencite{Bailey_2026}:
          \begin{equation}I_{GNorm} = \sqrt{\frac{N_{2 \text{ electorate}}}{N_{2 \text{ electorate}} + 1}\sum_{i = 1}^{n}\left(s_i - v_i\right)^2}.\end{equation}
\end{longtable*}

\begin{figure}[H]

{\centering \includegraphics{cbfiles/distdisp-1.pdf}

}

\caption{\label{fig:distdisp}Distribution of Disproportionality
Measures}
...
\texttt{link\_coalitionagreements}, \texttt{link\_positions\_pwib},
\texttt{link\_integrated\_state\_election\_surveys},
\texttt{link\_statepol\_ppg}, \texttt{link\_statepol\_cabinets} and
...
{\centering \includegraphics{cbfiles/printmanifestoplot-1.pdf}

}

\caption{\label{figure:manifestoavailability}Availability of manifestos
from polidoc.net and abgeordnetenwatch.de}
...
provided in the PWIB data file
(\texttt{ReplicationData\_Bräuninger\_etal\_2020.dta}) are not
...
\parencite{scheuchGermanFederalState2015}. Currently linked to StatePol
version
...
\subsection{\texorpdfstring{\texttt{link\_statepol\_ppg}}{link\_statepol\_ppg}}\label{link_statepol_ppg}

Dataset providing a link between \texttt{ltw\_elections} (or
\texttt{ltw\_combined}) and data on parliamentary party groups in the
StatePol database \parencite{kochStatePolDatenbankMitgliedern2024a}.

Use the \texttt{ppg} variable in the StatePol PPG dataset (not the
\texttt{ppg\_id} variable) to link StatePol and \texttt{bundeslaendeR}
data. Party groups that are founded later in the legislative cycle
cannot be linked to electoral outcome data. See \texttt{link\_notes} for
details on such cases.

The Bündnis 90/Die Grünen (AL) / UFV party group links to the election
results of both Bü90GrüUFV and Grüne in the 1990 Berlin state elections
as MdLs running on both lists formed a joint party group immediately
following the election.

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{landtag\_state\_abb} &\textbf{State name abbreviation in StatePol}\newline 
State name abbreviation in StatePol dataset.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{electoralperiod} &\textbf{State electoral period in StatePol}\newline 
State electoral period in StatePol dataset. Note that this count does not neccesarily match the number \texttt{state\_election\_term}. See the describtion of the \texttt{state\_election\_term} variable above.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{ppg} &\textbf{Name of Parliamentary Party Group on StatePol}\newline 
Name of Parliamentary Party Group on StatePol. Note that the \texttt{ppg} column is different from the \texttt{ppg\_id} column in the original StatePol dataset.
\end{longtable*}

\newpage

\subsection{\texorpdfstring{\texttt{link\_statepol\_cabinets}}{link\_statepol\_cabinets}}\label{link_statepol_cabinets}

Dataset providing links between government data in
\texttt{ltw\_governments} and \texttt{ltw\_combined} and cabinet data
from StatePol database
\parencite{kochStatePolDatenbankMitgliedern2024a}.

In some cases one cabinet in the StatePol database is assigned two
\texttt{gov\_ids} in \texttt{bundeslaendeR} data as StatePol covers
parties leaving a coalition using time variant cabinet membership of
party ministers whereas \texttt{bundeslaendeR} creates a new government
entry. In these cases there are two values in the \texttt{gov\_id}
column, separated by a semi-colon.
...
\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{cabinet} &\textbf{Cabinet name in StatePol}\newline 
Cabinet name in StatePol dataset.
\end{longtable*}

\newpage

\subsection{\texorpdfstring{\texttt{link\_stateparl}}{link\_stateparl}}\label{link_stateparl}

Dataset providing a link between \texttt{ltw\_elections} (or
\texttt{ltw\_combined}) and data on parliamentary speeches from the
StateParl database (paragraphs dataset) \parencite{baltermann_2025}.
Party groups that are founded later in the legislative cycle cannot be
linked to electoral outcome data. See \texttt{link\_notes} for details
on such cases. Note that StateParl treats the time before and after the
re-election of the Berlin Abgeordnetenhaus in 2023 (re-running the 2021
election) as the same electoral period. In the \texttt{bundeslaendeR}
data the \texttt{state\_election\_term} count gets incremented. The
\texttt{election\_date} variable in \texttt{link\_stateparl} thus
contains both election dates for this electoral period, separated by a
semi-colon. Also use the the \texttt{date} variable from StateParl's
paragraphs dataset to link the data for this electoral period. Note that
StateParl's \texttt{affiliation} variable denotes the political position
instead of the party affiliation for some speeches, in particular for
members of the state parliament presidency and -more crucially- for
members of the state government. \texttt{link\_stateparl} thus cannot
not help with linking these politicians' speeches to party level
election results, etc. Please refer to the StateParl codebook for more
information.

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{state} &\textbf{State name abbreviation in StateParl}\newline 
State name abbreviation in StateParl dataset.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{period} &\textbf{State electoral period in StateParl}\newline 
State electoral period in StateParl dataset.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{affiliation} &\textbf{Party affiliation in StateParl}\newline 
Speakers' party affiliation in StateParl dataset.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{state\_bundeslaender} &\textbf{State name abbreviation in bundeslaendeR}\newline 
State name abbreviation in bundeslaendeR.
\end{longtable*}

\newpage

\subsection{\texorpdfstring{\texttt{link\_gerda}}{link\_gerda}}\label{link_gerda}

Dataset providing a link between \texttt{ltw\_elections} (or
\texttt{ltw\_combined}) and state election results at the municipal
level from GERDA - German Election Database
\parencite{Heddesheimer2025GERDA}.

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{state\_gerda} &\textbf{State number in GERDA data}\newline 
State number in GERDA data.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{state} &\textbf{State name abbreviation in bundeslaendeR}\newline 
State name abbreviation in bundeslaendeR.
\end{longtable*}
...
\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{partyname\_gerda} &\textbf{Party name in GERDA data}\newline 
Party name in GERDA data.
\end{longtable*}

\begin{longtable*}{p{3.2cm}| p{11cm}}
\texttt{partyname\_short} &\textbf{Abbreviated Party Name}\newline 
Party abbreviation in bundeslaendeR data.
\end{longtable*}
